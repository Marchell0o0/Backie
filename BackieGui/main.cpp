...
int main(int argc, char* argv[]) {
	...
	if (IsRunningAsAdmin()) {
		SPDLOG_DEBUG("Running with admin rights");
	} else {
		SPDLOG_DEBUG("Running without admin rights");
	}
	...
	// TODO: Provide a way to use the program without TS
	SPDLOG_INFO("Connecting to task scheduler");
	TaskScheduler& TS = TaskScheduler::getInstance();
	if (!TS.initialize()) {
		SPDLOG_ERROR("Couldn't connect to task scheduler");
		return 1;
	}
	...
	std::shared_ptr<MonthlySchedule> monthly
		= std::make_shared<MonthlySchedule>();
	monthly->day = 20;
	monthly->hour = 9;
	monthly->minute = 0;
	...
	// /* Examples of TemplateVec and TemplateBuilder
	// ------------------------------------------------
	...
	auto newTemplate = builder.setName("Minecraft")
						   .setActiveTS(true)
						   .setDests({"W:\\Backie backups\\Dest 1"})
						   .setSrcs({"W:\\Src folder 3"})
						   .setSchedules({once})
						   .build();
	if (newTemplate) {
	...
	// auto templateCopy
	//  = builder.startFrom(*templates.begin()).setActiveTS(true).build();

	//  if (templateCopy) {
	//   templates.change(templates.begin(), *templateCopy);
	//  }
	// ------------------------------------------------
	// */
...
// 	Settings& settings = Settings::getInstance();
// 	Destination test_dest1("Default destination 1", "W:\\Backie backups\\Dest
// 1"); 	settings.addUpdate(test_dest1);

// 	Destination test_dest2("Default destination 2", "W:\\Backie backups\\Dest
// 2"); 	settings.addUpdate(test_dest2);
...
// 	std::shared_ptr<MonthlySchedule> monthlyTest =
// std::make_shared<MonthlySchedule>(); 	monthlyTest->day = 31;
// monthlyTest->hour = 9; 	monthlyTest->minute = 0;
